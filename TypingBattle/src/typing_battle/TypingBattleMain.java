...
public class TypingBattleMain {
	public static void main(String[] args) {
		...
		while(true) {
			System.out.println("Typing Battle에 오신것을 환영합니다.");
			...
			if (select == 1) {																//로그인
				...
				if(dev.getDevPw().equals(pw)) {
					...
					//로그인이후 프로세스
					while(true) {
						System.out.println(dev.getDevId()+"님 레벨: "+dev.getDevMoney()+" 소지금: "
								+UtilClass.strMoney(dev.getDevMoney()));
						System.out.println("1. 타자전투| 2. 상점 | 3. 명예의 전당 | 4. 종료");
						System.out.print(">>>");
						...
						if(select == 1) {
							//  타자전투
							System.out.println("1. 신입|2. 초급(Lv.10)|3. 중금(Lv.20) | 4. 고급(Lv.30)| 5.나가기");
							System.out.print(">>>");
							...
							if(select == 1) {
								...
								if(dev.getDevLevel()<10) {
									System.out.println("레벨 10 이상부터 입장 가능");
									continue;
								}else {
								...
								if(dev.getDevLevel()<20) {
									System.out.println("레벨 20 이상부터 입장 가능");
									continue;
								}else {
									wordList = service.getWordList(2);
									service.playTyping(dev, wordList, sc);
								...
								if(dev.getDevLevel()<30) {
									System.out.println("레벨 30 이상부터 입장 가능");
									continue;
								}else {
									wordList = service.getWordList(2);
									service.playTyping(dev, wordList, sc);
							...
							for(int i=0; i<devList.size(); i++) {
								System.out.println((i+1)+", "+devList.get(i).toString());
							}
							System.out.println("=============================");
